...
def test_get_valuations_violating_invariant1():
    M = DistLockModel('M1')
    S = M.get_state('pre')

    cand_invar = lambda M, S: \
        QForAll([Node, Epoch],
        lambda n, e: S.locked(e, n))

    cg = CEXGen(DistLockModel)
    cex = cg.get_pos_cex(cand_invar)
    assert cex.exists()

    valuations = cex.get_valuations_violating_invariant(cex.S, cand_invar, all=True)
    # for this protocol, we're asking for pos_cex (which currently only refers to initial state)
    # where locked(e, n) is universally false.
    # so the number of valuations we'll get will be |E| * |N|
    epoch_size = len(cex.M.get_universe(Epoch))
    node_size = len(cex.M.get_universe(Node))
    assert len(valuations) == epoch_size * node_size

def test_get_valuations_violating_invariant2():
    M = DistLockModel('M1')
    S = M.get_state('pre')
    ...
    valuations = cex.get_valuations_violating_invariant(cex.S, cand_invar, all=True)
    # pos_cex == initial state.
    # there's exactly one node where held is true in initial state,
    # so number of valuations should be equal to (|N|-1) * |E|
    epoch_size = len(cex.M.get_universe(Epoch))
    node_size = len(cex.M.get_universe(Node))
    assert len(valuations) == (node_size - 1) * epoch_size

def test_get_valuations_violating_invariant3():
    M = DistLockModel('M1')
    S = M.get_state('pre')
    ...
    valuations = cex.get_valuations_violating_invariant(cex.S, cand_invar, all=True)
    # pos_cex == initial state.
    # ep(n) == one or zero, depending upon whether n = first or other node.
    # so the invariant will be false for (first, all_epochs_except_one) and (other, all_epochs_except_zero)
    epoch_size = len(cex.M.get_universe(Epoch))
    node_size = len(cex.M.get_universe(Node))
    assert len(valuations) == (epoch_size - 1) + (node_size-1)*(epoch_size-1)

#cex = test_2()
# %%

# The stuff below is when our protocol modelling was buggy.
# We thought we'd found the invariants, but we hadn't!
