...
ID3 = ['--no-id3', '--id3']
ITER_DEEP = ['--iter-deep'] # ['--no-iter-deep', '--iter-deep']
PROTOCOL = ['ricart_agrawala'] # ['dist_lock'] # ricart_aggarwal not in this list because it's on a different branch currently.
...
for i, (cheap, id3, iter_deep, protocol) in enumerate(tqdm(variants)):
    run_name = f'eval_{i}_{cheap}_{id3}_{iter_deep}_{protocol}'
